...
import { ResponsiveContainer, Treemap, Tooltip, PieChart as RechartsPieChart, Cell, BarChart, Bar, XAxis, YAxis, CartesianGrid } from 'recharts';
...
export function AreaTaxonomy() {
  const [expandedNodes, setExpandedNodes] = useState<Set<string>>(new Set(['Natural Science']));
  const [searchTerm, setSearchTerm] = useState('');
  const [selectedDomain, setSelectedDomain] = useState<string>('all');
  const [activeView, setActiveView] = useState<'tree' | 'treemap' | 'sunburst'>('tree');

  // Flatten the taxonomy for search and statistics
  const flattenTaxonomy = (node: TaxonomyNode, path: string[] = [], depth = 0): Array<{
    id: string;
    name: string;
    path: string[];
    depth: number;
    type: 'domain' | 'field' | 'subfield' | 'specialization';
    ...
    if (typeof node === 'object' && node !== null) {
      Object.entries(node).forEach(([key, value]) => {
        ...
        result.push({
          ...
          path: currentPath,
          depth,
          type: isDomain ? 'domain' : isField ? 'field' : isSubfield ? 'subfield' : 'specialization',
          childrenCount: typeof value === 'object' ? Object.keys(value).length : 0,
          domain: path[0] || key
        });
  ...
  const calculateStatistics = (): AreaStats => {
    ...
    // Calculate per-domain counts (simplified - in real app, you'd have actual data)
    const domainStats = domains.map((domain, index) => {
      const domainAreas = allAreas.filter(area => area.domain === domain);
      const colors = ['#10b981', '#3b82f6', '#8b5cf6', '#ef4444', '#f59e0b', '#ec4899', '#06b6d4'];
      return {
        name: domain,
    ...
    return {
      ...
      totalResearchers: 1248, // Mock data - replace with actual
      totalPublications: 42892, // Mock data - replace with actual
      maxDepth,
      domains: domainStats
    };
  };
  ...
  const renderTaxonomyNode = (node: TaxonomyNode, parentKey = '', depth = 0, domain = ''): JSX.Element[] => {
    ...
    Object.entries(node).forEach(([key, value]) => {
      ...
      // Filter by domain
      const isDomainMatch = selectedDomain === 'all' || currentDomain === selectedDomain;
      ...
      const icons = [Network, Cpu, FlaskRound, BookOpen, Globe, BarChart3];
      ...
      nodes.push(
        <div key={nodeKey} className="mb-1">
          ...
          >
            ...
            <div
              className={`w-10 h-10 rounded-lg flex items-center justify-center bg-gradient-to-br ${depthColors[depth % depthColors.length]}`}
            >
              ...
              <div className="text-emerald-400/60 text-sm flex items-center gap-4 mt-1 flex-wrap">
                {hasChildren && (
                  <span className="flex items-center gap-1 bg-emerald-500/10 px-2 py-0.5 rounded">
                    ...
                    {Object.keys(value).length} sub-areas
                  </span>
                )}
  ...
  const treemapData = Object.entries(taxonomyData.taxonomy).map(([domain, fields]) => {
    const areaCount = allAreas.filter(area => area.domain === domain).length;
    return {
      name: domain,
      size: areaCount,
        ...
        return (
          <div className="bg-[#0f2820] border border-[#1a3d33] rounded-xl p-6">
            ...
            <ResponsiveContainer width="100%" height={400}>
              <RechartsPieChart>
                <Tooltip 
                  formatter={(value) => [`${value} areas`, 'Count']}
                  contentStyle={{ 
      ...
      <div className="flex items-center gap-2 text-sm">
        {parts.map((part, index) => (
          <>
            ...
            {index < parts.length - 1 && <ChevronRight className="w-4 h-4 text-emerald-400/40" />}
          </>
        ))}
      </div>
    );
    ...
    <div className="p-6">
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl text-white mb-2 font-bold">Research Area Taxonomy</h1>
            <p className="text-emerald-400/60">
              {taxonomyData.metadata.description} • {taxonomyData.metadata.total_domains} domains
            </p>
          </div>
          <div className="px-4 py-2 bg-emerald-500/10 border border-emerald-500/30 rounded-lg">
            <span className="text-emerald-400 text-sm">Version {taxonomyData.metadata.version}</span>
          </div>
        </div>
        
        <div className="flex flex-wrap gap-2 mt-4">
          {taxonomyData.metadata.sources.map((source, index) => (
            <span 
              key={index}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div className="bg-gradient-to-br from-emerald-500/10 to-emerald-600/10 border border-emerald-500/30 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="text-3xl text-white font-bold">{stats.totalAreas}</div>
            </div>
        ...
        <div className="bg-gradient-to-br from-blue-500/10 to-blue-600/10 border border-blue-500/30 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="text-3xl text-white font-bold">{stats.maxDepth + 1}</div>
            </div>
        ...
        <div className="bg-gradient-to-br from-purple-500/10 to-purple-600/10 border border-purple-500/30 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <div className="text-3xl text-white font-bold">{stats.domains.length}</div>
            </div>
        ...
        <div className="bg-gradient-to-br from-cyan-500/10 to-cyan-600/10 border border-cyan-500/30 rounded-xl p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="text-cyan-400/60 text-sm">Sources</div>
              <div className="text-3xl text-white font-bold">{taxonomyData.metadata.sources.length}</div>
            </div>
      ...
      {/* Domain Statistics */}
      <div className="bg-[#0f2820] border border-[#1a3d33] rounded-xl p-6 mb-6">
        <h3 className="text-white mb-6 text-lg font-semibold">Domain Statistics</h3>
          ...
          <div>
            <div className="space-y-4">
              {stats.domains.map((domain, index) => {
                const percentage = Math.round((domain.count / stats.totalAreas) * 100);
                return (
                  <div key={domain.name} className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-white text-sm">{domain.name}</span>
                      <span className="text-emerald-400 text-sm">{domain.count} areas ({percentage}%)</span>
                    </div>
      ...
      <div className="mt-6 bg-gradient-to-br from-emerald-500/10 to-teal-500/10 border border-emerald-500/30 rounded-xl p-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h4 className="text-emerald-400 mb-2 text-lg font-semibold">LLM-Enhanced Taxonomy</h4>
            <p className="text-emerald-400/80">
              This unified taxonomy integrates multiple established classification systems with AI-generated 
              hierarchies from leading language models (Claude, GPT-4, DeepSeek, Qwen). The dynamic structure 
              adapts to emerging research trends and provides granular classification for fair cross-disciplinary 
              assessment. Regular updates ensure coverage of cutting-edge fields like quantum computing, 
